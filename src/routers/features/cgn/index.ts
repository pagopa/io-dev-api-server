...
export const CGNPlugin: Plugin<CGNPluginOptions> = async (
  ...
  handleRoute("post", addPrefix("/activation"), (_, res) => {
   ...
   */
  handleRoute("get", addPrefix("/activation"), (_, res) =>
    // if there is no previous activation -> Request created -> send back the created id
    pipe(
      O.fromNullable(idActivationCgn),
      O.fold(
  ...
  // 404 -> no CGN found
  handleRoute("get", addPrefix("/status"), (_, res) => {
    if (isCgnActivated()) {
      currentCGN = {
  ...
  handleRoute("post", addPrefix("/eyca/activation"), (_, res) => {
    // if there is no previous activation -> Request created -> send back the created id
    pipe(
   ...
   * status 404 means no activation job has been found
   */
  handleRoute("get", addPrefix("/eyca/activation"), (_, res) =>
    // if there is no previous activation -> Request created -> send back the created id
    pipe(
      O.fromNullable(idActivationEyca),
      O.fold(
  ...
  handleRoute("get", addPrefix("/eyca/status"), (_, res) => {
  ...
  handleRoute("post", addPrefix("/otp"), (_, res) => {
    const now = new Date().getTime();
  ...
  handleRoute("post", addPrefix("/delete"), (_, res) => {
