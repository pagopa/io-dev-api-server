...
export const PaymentPlugin: Plugin<PaymentPluginOptions> = async (
  ...
  handleRoute(
    ...
    (_, res) => {
      pipe(
        O.fromNullable(options.wallet.verificaError),
        O.fold(
          () => {
   ...
   * STEP 2
   */
  handleRoute("post", addApiV1Prefix("/payment-activations"), (req, res) => {
  ...
  handleRoute("post", "/wallet/v3/webview/transactions/cc/verify", (req, res) =>
  ...
  // payment
  handleRoute("post", "/wallet/v3/webview/transactions/pay", (req, res) =>
    handlePaymentPostAndRedirect(req, res, options.wallet.paymentOutCode)
  );
